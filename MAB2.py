...
####################################################
# Given arm a equals to 1 or 2, this function returns a Bernouli r.v.
# with either p=0.75 or p=0.25. At time i, if this is after
# half of the time horizon T, the the reward of the arm will switch.
def Bern(a,i,T):
	if i < T/2.:
		if a == 1:
			rand = np.random.rand()
...
def eGreedy(T):
	...
	n1, n2 = 0., 0.
	def pull1(i,T):
		return n1 + 1, Bern(1,i,T) + x1
	def pull2(i,T):
		return n2 + 1, Bern(2,i,T) + x2
	...
	# calculate regret 
	R = 0.5 * T - x1 - x2
	# calculate regret relative to single best action in hindsight
	Rh = 0.75 * T - x1 - x2
	return Rh
...
def ThomSam(T):
	...
	for i in range(T):
		...
		if theta1 >= theta2:
			n1, x1, success = pull1(i,T)
			if success == 1:
				s1 += 1
			else:
				f1 += 1
		else:
			n2, x2, success = pull2(i,T)
			if success == 1:
				s2 += 1
			else:
				f2 += 1
...
if __name__ == '__main__':
	...
	# g = plt.semilogx(ts,r4s, 'go', label='Exp3') #exp
 ...
 #           ncol=3,
 #           fontsize=10)
	# plt.savefig('GeneralSemiLog_2.jpg')
